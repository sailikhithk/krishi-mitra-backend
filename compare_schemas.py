...
from app.models import User, SoilHealth, Bid, Scheme, WeatherData, MarketPrice
    ...
    for table_name in model_metadata.tables:
        if table_name not in current_metadata.tables:
            ...
            for column in model_columns - current_columns:
                print(f"Column {column} in table {table_name} is missing in the current database.")
            for column in current_columns - model_columns:
                print(f"Column {column} in table {table_name} is extra in the current database.")
            for column in model_columns & current_columns:
                if str(model_table.columns[column].type) != str(current_table.columns[column].type):
                    print(f"Column {column} in table {table_name} has different type. Expected: {model_table.columns[column].type}, Found: {current_table.columns[column].type}")
