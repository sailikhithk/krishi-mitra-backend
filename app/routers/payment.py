...
async def read_payments(
    ...
    if current_user.role == UserRole.BUYER:
        result = await session.execute(
            select(Payment)
            .join(Payment.bid)
            .where(Payment.bid.user_id == current_user.id)
            .offset(skip)
            .limit(limit)
        )
        ...
        result = await session.execute(
            select(Payment)
            .join(Payment.bid)
            .join(Payment.bid.produce_listing)
            .where(Payment.bid.produce_listing.farmer_id == current_user.id)
            .offset(skip)
            .limit(limit)
        )
    else:
...
async def read_payment(
    ...
    if (
        ...
        and payment.bid.produce_listing.farmer_id != current_user.id
    ):
        raise HTTPException(
            status_code=403, detail="Not authorized to view this payment"
        )
