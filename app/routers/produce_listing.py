...
from app.schemas.produce_listing import ProduceListingCreate
from app.schemas.produce_listing import ProduceListingRead
from app.schemas.produce_listing import ProduceListingUpdate
...
async def create_produce_listing(
    ...
    if current_user.role != UserRole.FARMER:
        raise HTTPException(
            status_code=403, detail="Only farmers can create produce listings"
        )

    new_produce_listing = ProduceListing(
        **produce_listing.dict(),
...
async def read_produce_listing(
    listing_id: int, session: AsyncSession = Depends(get_async_session)
):
