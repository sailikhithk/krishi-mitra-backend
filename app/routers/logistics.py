...
async def create_logistics(
    ...
    if current_user.role not in [UserRole.FARMER, UserRole.BUYER]:
        raise HTTPException(
            status_code=403, detail="Not authorized to create logistics"
        )
...
async def read_logistics(
    ...
    result = await session.execute(
        select(Logistics)
        .where(
            (Logistics.from_user_id == current_user.id)
            | (Logistics.to_user_id == current_user.id)
        )
        .offset(skip)
        .limit(limit)
    )
...
async def read_logistics_by_id(
    ...
    if current_user.id not in [logistics.from_user_id, logistics.to_user_id]:
        raise HTTPException(
            status_code=403, detail="Not authorized to view this logistics"
        )
    return logistics
...
async def update_logistics(
    ...
    if current_user.id not in [
        existing_logistics.from_user_id,
        existing_logistics.to_user_id,
    ]:
        raise HTTPException(
            status_code=403, detail="Not authorized to update this logistics"
        )
...
async def delete_logistics(
    ...
    if current_user.id not in [logistics.from_user_id, logistics.to_user_id]:
        raise HTTPException(
            status_code=403, detail="Not authorized to delete this logistics"
        )
