...
from create_db import Bid
from create_db import engine
from create_db import Logistics
from create_db import Payment
from create_db import ProduceCategory
from create_db import ProduceListing
from create_db import Scheme
from create_db import SoilHealth
from create_db import User
from create_db import user_scheme
from create_db import UserRole
        ...
        async with session.begin():
            ...
            users = [
                User(
                    ...
                    email="john@krishimitra.com",
                    hashed_password=get_password_hash("1234"),
                    role=UserRole.FARMER,
                    farm_size=150.5,
                    location="Anantapur",
                    phone_number="+911234567890",
                    aadhar_card_url="http://example.com/aadhar/john.jpg",
                ),
                User(
                    username="jane_buyer",
                    email="jane@krishimitra.com",
                    hashed_password=get_password_hash("1234"),
                    role=UserRole.BUYER,
                    company_name="Jane's Agro",
                    business_type="Wholesaler",
                    phone_number="+919876543210",
                    aadhar_card_url="http://example.com/aadhar/jane.jpg",
                ),
                User(
                    username="bob_farmer",
                    email="bob@krishimitra.com",
                    hashed_password=get_password_hash("1234"),
                    role=UserRole.FARMER,
                    farm_size=200.0,
                    location="Chittoor",
                    phone_number="+917890123456",
                    aadhar_card_url="http://example.com/aadhar/bob.jpg",
                ),
                User(
                    username="alice_admin",
                    email="alice@krishimitra.com",
                    hashed_password=get_password_hash("1234"),
                    role=UserRole.ADMIN,
                    department="IT",
                    access_level="Full",
                    phone_number="+915678901234",
                    aadhar_card_url="http://example.com/aadhar/alice.jpg",
                ),
            ]
            ...
            produce_listings = [
                ProduceListing(
                    ...
                    pickup_location=random.choice(
                        ["Farm Gate", "Local Mandi", "Warehouse"]
                    ),
            ...
            bids = [
                Bid(
                    ...
                    price=round(
                        random.uniform(listing.base_price, listing.base_price * 1.5), 2
                    ),
            ...
            for user in users:
                if user.role == UserRole.FARMER:
                    user_schemes = random.sample(
                        schemes, random.randint(1, len(schemes))
                    )
                    for scheme in user_schemes:
                        await session.execute(
                            user_scheme.insert().values(
